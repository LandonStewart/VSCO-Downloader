...
class Scraper(object):
    ...
    def getJournal(self):
        ...
        self.pbarj = tqdm(
            total=self.totalj,
            desc='Downloading journal posts of %s' %
            self.username,
            unit=' posts')
        for x in self.works:
    ...
    def getJournalList(self):
        ...
        self.jour_found = self.session.get(
            ...
            params={
                "size": 10000,
                "page": 1},
            headers=media).json()["articles"]
        self.pbarjlist = tqdm(
            desc='Finding new journal posts of %s' %
        ...
        for item in self.jour_found[loc]["body"]:
            ...
            if item['type'] == "image":
                if os.path.exists(
                    ...
                    if '%s.jpg' % str(
                        item["content"][0]["id"]) in os.listdir(
                        os.path.join(
                            os.getcwd(),
                            self.jour_found[loc]["permalink"])):
                        continue
        ...
        if lists[2] == "img":
            ...
            with open('%s.mp4' % str(lists[1]), 'wb') as f:
                for chunk in requests.get(
                        lists[0], stream=True).iter_content(
                        chunk_size=1024):
                    if chunk:
                        f.write(chunk)
        self.pbarj.update()
    ...
    def getImages(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            future_to_url = {
                executor.submit(
                    self.download_img_normal,
                    lists): lists for lists in self.imagelist}
            for future in tqdm(
                    concurrent.futures.as_completed(future_to_url),
                    total=len(
                        self.imagelist),
                    desc='Downloading posts of %s' %
                    self.username,
                    unit=' posts'):
...
def main():
    ...
    parser.add_argument(
        ...
        '--multiple',
        action="store_true",
        help='Downloads multiple users')
    parser.add_argument(
        '-mj',
        '--multipleJournal',
        action="store_true",
        help='Downloads multiple users journal')
    parser.add_argument('-a', '--all', action="store_true",
                        help='Downloads multiple users journals and images')
